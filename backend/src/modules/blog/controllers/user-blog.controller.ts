...
export class UserBlogController {
  ...
  @ApiOperation({ summary: 'Get one of my blog posts (all statuses)' })
  @Get(':id')
  getMyBlog(@CurrentUser() user: JwtPayload, @Param('id') id: string) {
    return this.blogService.getUserBlog(user.sub, id);
  }

...
export class ActivityLogsController {
  ...
  @Get()
  async getByDate(@CurrentUser() user: JwtPayload, @Query() query: ActivityLogQueryDto) {
  ...
  @Get('range')
  async getRange(@CurrentUser() user: JwtPayload, @Query() query: ActivityLogQueryDto) {
  ...
  @ApiOperation({ summary: 'Log steps count' })
  @Patch('steps')
  async logSteps(@CurrentUser() user: JwtPayload, @Body() dto: LogStepsDto) {
    return this.activityLogsService.logSteps(user.sub, dto);
  }

  @ApiOperation({ summary: 'Log calories burned' })
  @Patch('calories-burned')
  async logCaloriesBurned(@CurrentUser() user: JwtPayload, @Body() dto: LogCaloriesBurnedDto) {
